...
poll() no longer self-marks (C1 fix, see test_scheduler_mark_on_success.py) —
a caller must invoke mark_fired(name, now) once a job succeeds. These tests
simulate "the job ran and succeeded" with _poll_and_mark, mirroring exactly
how SessionRunner.run_once drives the scheduler in production."""
...
def test_each_job_fires_only_once_per_day():
    ...
    s = LifecycleScheduler()
    assert PRE_OPEN_SYNC in _poll_and_mark(s, _dt(8, 31))
    assert s.poll(_dt(8, 32)) == [], "already-fired jobs do not re-fire same day"


def test_poll_alone_does_not_mark_and_retries_until_marked():
    """The new C1 contract, exercised directly here (not just in the dedicated
    test_scheduler_mark_on_success.py file): a bare poll() must never advance
    state — the same job stays due across repeated polls until mark_fired."""
    from autotrader.scheduler import LifecycleScheduler, PRE_OPEN_SYNC
    s = LifecycleScheduler()
    assert PRE_OPEN_SYNC in s.poll(_dt(8, 31))
    assert PRE_OPEN_SYNC in s.poll(_dt(8, 32)), "unmarked -> still due"
    s.mark_fired(PRE_OPEN_SYNC, _dt(8, 32))
    assert PRE_OPEN_SYNC not in s.poll(_dt(8, 33))
...
def test_single_job_fires_at_its_time():
    ...
    s = LifecycleScheduler()
    _poll_and_mark(s, _dt(10, 0))            # fire the two morning jobs (PRE_OPEN_SYNC, ENTRY_OPEN)
    _poll_and_mark(s, _dt(13, 0))            # fire REBALANCE (12:30 past due)
    _poll_and_mark(s, _dt(14, 0))            # fire RISK_CHECK_MID (13:30 past due)
    _poll_and_mark(s, _dt(15, 0))            # fire RISK_CHECK_LATE (15:00 exactly)
...
def test_eod_flatten_fires_after_1615():
    ...
    s = LifecycleScheduler()
    _poll_and_mark(s, _dt(16, 0))           # nothing new yet beyond morning catch-up
...
def test_new_day_resets_all_jobs():
    from autotrader.scheduler import LifecycleScheduler, PRE_OPEN_SYNC
    s = LifecycleScheduler()
    _poll_and_mark(s, _dt(8, 31, day=12))   # fire PRE_OPEN_SYNC on the 12th
...
def test_eod_report_fires_after_1630_and_once_per_day():
    ...
    s = LifecycleScheduler()
    _poll_and_mark(s, _dt(16, 20))                     # nothing new at 16:20 beyond catch-up
    due = _poll_and_mark(s, _dt(16, 31))
...
def test_at_most_once_jobs_survive_restart():
    ...
    REBALANCE (review Minor #8) — their last-fired dates persist via
    mark_fired, which is what the runner calls once each job succeeds."""
    from datetime import datetime
    from zoneinfo import ZoneInfo
    ...
    fired = _poll_and_mark(s1, datetime(2026, 7, 6, 16, 31, tzinfo=_NY))
...
def test_at_most_once_fires_fresh_next_day():
    ...
    store = {}
    s1 = LifecycleScheduler(state_get=store.get, state_set=store.__setitem__)
    _poll_and_mark(s1, datetime(2026, 7, 6, 16, 31, tzinfo=_NY))
    s2 = LifecycleScheduler(state_get=store.get, state_set=store.__setitem__)
