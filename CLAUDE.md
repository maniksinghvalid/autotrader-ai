...
The only first-party application code so far is `dashboard/` — a read-only Flask view of the paper account that delegates all Moomoo access to the vendored scripts. It is the reference example of how to build on top of the skills correctly.
...
├── PROJECT_INSTRUCTIONS_RECOMMENDED.md       # Source draft these project rules derive from
├── config/
│   └── dashboard.config.example              # Non-secret presentation settings (copy → dashboard.config)
├── dashboard/                                # Read-only Flask ops dashboard (first-party app)
│   ├── server.py                             # Backend: health gate + /api/* endpoints; READ-ONLY allow-list
│   ├── opend_ready.py                        # is_opend_ready() exponential-backoff readiness gate
│   ├── static/index.html                     # Single-page live-refreshing frontend
│   ├── requirements.txt                      # flask>=3.0 (moomoo-api comes from the install skill)
│   └── README.md
└── skills/
    ...
    │   ├── SKILL.md
    │   └── scripts/                          # install_{mac,linux,win}.md, detect_version.md,
    │                                         # verify_version.md  (these are .md guides, not .py)
    └── moomooapi/                            # Skill: trading, market data, subscriptions
        ...
        └── scripts/
            ├── common.py                     # Shared env checks; every script imports this
            ├── quote/                        # ~62 scripts: snapshots, klines, order book,
            │                                 # options, financials, screeners, plates, etc.
            ├── trade/                        # ~22 scripts: place/cancel/modify orders,
            │                                 # portfolios, fees, crypto trading
            └── subscribe/                    # ~10 scripts: real-time streaming pushes
```

Folders that **do not yet exist** but may be added later: `strategies/`, `main.py`. Anything new must follow the rules in the "New Code" section below.
...
A read-only Flask app that shows the paper account (summary, positions, open orders, today's fills). It is the working reference for composing on top of the skills: it never calls trade/order/unlock scripts, only the four read scripts in an allow-list, and gates every endpoint on the OpenD health check.

Run it (pip or uv — `dashboard/requirements.txt` pins both `flask>=3.0` and `moomoo-api>=10.4.6408`):

```bash
# pip:
pip install -r dashboard/requirements.txt && python dashboard/server.py

# uv (isolated env; works because server.py invokes scripts via sys.executable):
uv venv && uv pip install -r dashboard/requirements.txt && uv run python dashboard/server.py
```

Serves http://127.0.0.1:8787/. Either path installs the **Python SDK only** — the OpenD GUI is a native app installed via `/install-moomoo-opend`, which also writes the `~/.moomoo_skill_version` stamp (`common.py`'s stamp/version check is warn-only and non-blocking, so a uv-installed SDK just emits a one-line warning).

Architecture notes that any new app should mirror:

- **`server.py` enforces read-only via `ALLOWED_SCRIPTS`** — a hardcoded allow-list of exactly `get_accounts`, `get_portfolio`, `get_orders`, `get_order_fill_list` (all under `skills/moomooapi/scripts/trade/`). It shells out to these scripts via `subprocess`; any script not in the list is refused. This is how "the dashboard can never place or cancel an order" is structurally guaranteed, not just promised.
- **`opend_ready.py` is the readiness gate.** `is_opend_ready()` polls the OpenD TCP endpoint with exponential backoff (no `time.sleep()` as a check). Endpoints return **503** until OpenD is ready rather than serving stale/empty data. Reuse this module rather than reinventing readiness logic.
- **Endpoints:** `/api/health`, `/api/accounts`, `/api/snapshot` (auto-detects the first SIMULATE account if `acc_id` not given).
- **Config precedence:** OpenD host/port/account come from `FUTU_OPEND_HOST` / `FUTU_OPEND_PORT` / `FUTU_ACC_ID` env vars (read through the vendored `common.get_config()`). Presentation-only settings (`web_port`, `refresh_seconds`, `opend_ready_timeout`, `preferred_market`) come from `config/dashboard.config` (copy from `config/dashboard.config.example`) or `DASHBOARD_*` env vars. No hosts/ports/accounts are hardcoded.
- **Keep `refresh_seconds >= 20`:** each refresh makes ~4 `refresh_cache=True` calls, and Moomoo caps refreshing queries at 10 per 30s per account (see `docs/API_LIMITS.md`).
...
- Today `config/` holds only `dashboard.config.example` (non-secret presentation settings). Secrets are not stored here yet — OpenD connection/account come from `FUTU_*` env vars.
